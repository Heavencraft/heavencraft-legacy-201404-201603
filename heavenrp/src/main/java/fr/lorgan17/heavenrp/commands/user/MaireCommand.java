	...
	protected void onConsoleCommand(CommandSender sender, String[] args) throws HeavenException
	{
		switch (args.length)
		{
			...
			case 3:
				...
				User mayor = UsersManager.getByName(Utils.getExactName(args[2]));
				
				if (args[1].equalsIgnoreCase("+"))
				{
					TownsManager.addMayor(args[0], mayor);
					Utils.sendMessage(sender, "Le joueur {%1$s} est désormais maire de la ville {%2$s}.", mayor.getName(), args[0]);
				}

				else if (args[1].equalsIgnoreCase("-"))
				{
					TownsManager.removeMayor(args[0], mayor);
					Utils.sendMessage(sender, "Le joueur {%1$s} n'est désormais plus maire de la ville {%2$s}.", mayor.getName(), args[0]);
				}
