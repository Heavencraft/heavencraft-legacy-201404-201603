...
public class Colis
{
	...
	public Colis(int ID)
	{
				...
				"SELECT * FROM `poste_colis`  WHERE `IDcolis` = ? AND `isLOG` = 0"))
		{
			...
			final String expID = rs.getString("expediteur");
			final String destID = rs.getString("destinataire");
	...
	public void envoyer()
	{
		// TODO: Pour economiser de la plawce, mettre le string inventaire en
		// Base 64
		try (PreparedStatement ps = HeavenRP.getConnection().prepareStatement(
				"INSERT INTO poste_colis (expediteur, destinataire, dateEnvoi, contenu, isLOG) VALUES (?, ?, NOW(), ?, ?)"))
		{
			...
			try
			{
				PlayerUtil.getPlayer(this.destinataire).sendMessage(
						String.format(FORMAT_POSTE, "Vous avez recu un colis, vous pouvez le récuperer a la poste!"));
			}
			catch (final PlayerNotConnectedException e)
			{
			}
	...
	{
		for (final ItemStack its : this.contenu.getContents())
		{
			if (its != null)
		...
		try (PreparedStatement ps = HeavenRP.getConnection().prepareStatement(
				"DELETE FROM poste_colis WHERE `IDcolis` = ? AND `isLOG` = 0"))
		{
