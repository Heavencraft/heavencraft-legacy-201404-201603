...
public class UsersManager
{
	public static class User
	{
		...
		private User(ResultSet rs) throws SQLException
		{
			...
			_bankBalance = rs.getInt("bank_balance");
		...
		private void update()
		{
			...
			{
				PreparedStatement ps = HeavenRP.getConnection().prepareStatement(
						"UPDATE users SET balance = ?, bank_balance = ?, last_login = ? WHERE id = ?;");
				ps.setInt(1, _balance);
				ps.setInt(2, _bankBalance);
				ps.setTimestamp(3, _lastLogin);
				ps.setInt(4, _id);
	...
	private static Map<String, User> _cache = new HashMap<String, User>();
	...
	public static User getByName(String name) throws HeavenException
	{
		User user = _cache.get(name);
	...
	public static void createUser(String name)
	{
		try
		{
			PreparedStatement ps = HeavenRP.getConnection()
