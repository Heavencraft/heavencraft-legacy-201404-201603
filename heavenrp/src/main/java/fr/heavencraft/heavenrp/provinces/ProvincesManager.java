...
public class ProvincesManager
{
	public static class Province
	{
		...
		/*private void update()
		{
			try
			{
				PreparedStatement ps = HeavenRP.getConnection().prepareStatement("UPDATE mayor_cities SET world = ?, x = ?, y = ?, z = ?, yaw = ?, pitch = ? WHERE id = ?");
				ps.setString(1, _warp.getWorld().getName());
				ps.setDouble(2, _warp.getX());
				ps.setDouble(3, _warp.getY());
				ps.setDouble(4, _warp.getZ());
				ps.setFloat(5, _warp.getYaw());
				ps.setFloat(6, _warp.getPitch());
				ps.setInt(7, _id);
	...
	public static Province getProvinceById(int id) throws HeavenException
	{
		try
		{
			PreparedStatement ps = HeavenRP.getConnection().prepareStatement("SELECT * FROM mayor_cities WHERE id = ? LIMIT 1");
			ps.setInt(1, id);
	...
	public static Province getProvinceByName(String name) throws HeavenException
	{
		try
		{
			PreparedStatement ps = HeavenRP.getConnection().prepareStatement("SELECT * FROM mayor_cities WHERE login = ? LIMIT 1");
			ps.setString(1, name);
	...
	public static Province getProvinceByUser(int id)
	{
		try
		{
			PreparedStatement ps = HeavenRP.getConnection().prepareStatement("SELECT mc.id, mc.login, mc.color, mc.world, mc.x, mc.y, mc.z, mc.yaw, mc.pitch FROM mayor_cities mc, mayor_people mp WHERE mc.id = mp.city_id AND mp.user_id = ? LIMIT 1");
			ps.setInt(1, id);
