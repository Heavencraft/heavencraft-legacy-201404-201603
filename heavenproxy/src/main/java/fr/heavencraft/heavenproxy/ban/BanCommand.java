...
public class BanCommand extends HeavenCommand {
	...
	public static void banPlayer(String playerName, String bannedBy, String reason)
	{
		BanManager.banPlayer(playerName, bannedBy, reason);
		LogsManager.addBan(bannedBy, playerName, reason);
		
		try
		{
			ProxiedPlayer player = Utils.getPlayer(playerName);

			KickManager.addReason(player.getName(), String.format(BAN, bannedBy, reason));
			player.disconnect(String.format(BAN_MESSAGE, bannedBy, reason));
		}
