...
public class OnlineLogListener implements Listener {
	...
	public OnlineLogListener(Plugin plugin)
	{
		this.plugin = plugin;
	...
	{
		if		(name.equals(NEXUS))		return NEXUS_ID;
		else if	(name.equals(SRP))			return SRP_ID;
		else if (name.equals(ORIGINES))		return ORIGINES_ID;
		else if (name.equals(CREAFUN))		return CREAFUN_ID;
		else if (name.equals(FACTIONS))		return FACTIONS_ID;
		else if (name.equals(ULTRAHARD))	return ULTRAHARD_ID;
		else if (name.equals(INFECTED))		return GAMES_ID;
		else if (name.equals(MARIOKART))	return GAMES_ID;
		else if (name.equals(TNTRUN))		return GAMES_ID;
		else if (name.equals(BACKBONE))		return GAMES_ID;
		else if (name.equals(PAINTBALL))	return GAMES_ID;
		else
		{
	...
	private static void UpdateDatabase(int serverId, String players, int nbPlayers)
	{
		try
		{
			PreparedStatement ps = HeavenProxy.getMainConnection().prepareStatement("UPDATE onlinelog SET playercount = ?, playernames = ? WHERE id = ?");
