...
public class RejoindreCommand extends Command
{
	...
	protected void onPlayerCommand(Player player, String[] args) throws MyException
	{
		...
		{
			Message.sendMessage(player, "Le joueur {" + otherPlayer.getDisplayName()
					+ "} n'est pas dans le même monde que vous.");
			return;
		}
		...
		Message.sendMessage(otherPlayer, "Le joueur {" + player.getDisplayName()
				+ "} souhaite se téléporter vers vous. Tapez {/accepter} pour accepter.");
		Message.sendMessage(player,
				"Une invitation de téléportation a été envoyée vers {" + otherPlayer.getDisplayName() + "}.");
	}
