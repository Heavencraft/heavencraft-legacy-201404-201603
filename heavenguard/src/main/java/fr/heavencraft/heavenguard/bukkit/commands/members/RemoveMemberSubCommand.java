...
public class RemoveMemberSubCommand implements SubCommand
{
	...
	public void execute(CommandSender sender, String[] args) throws HeavenException
	{
		...
		HeavenGuard.getRegionProvider().getRegionByName(name).removeMember(member.getUniqueId(), false);
		ChatUtil.sendMessage(sender, "{%1$s} n'est plus membre de la protection {%2$s}.", member.getName(), name);
	}

	@Override
	public void sendUsage(CommandSender sender)
	{
		ChatUtil.sendMessage(sender, "/{region} removemember <protection> <member>");
	}
}